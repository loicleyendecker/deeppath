...
def test_dget_repetitions():
    ...
    assert dget(data, f"deeply/nested[0]/path") == 2
    assert not dget(data, f"deeply/nested[10]/path")
    assert dget(data, f"deeply/nested[-1]/path") == 4
...
def test_dget_flatten_and_repetition():
    ...
    assert dget(data1, "*/nested_in_rep") is None  # */ is a list, it needs explicit unfold
